...
class MainManager(QWidget):
    ...
    def connectSignals(self):
        self.connect(self.pface, pyqtSignal("loadFetched(PyQt_PyObject)"), self.loadHistory)

        self.connect(self.animator, pyqtSignal("frameChanged(int)"), self.animate)
        self.connect(self.animator, pyqtSignal("finished()"), self.animateFinished)
        self.connect(self.ui.newSnapshotPB, pyqtSignal("clicked()"), self.takeSnapshot)
        self.connect(self.ui.buttonCancelMini, pyqtSignal("clicked()"), self.hideEditBox)
        self.connect(self.ui.aliasLE, pyqtSignal("textEdited(const QString &)"), self.setAlias)
        self.connect(self.ui.configurePB, pyqtSignal("clicked()"), self.showConfig)
    ...
    def loadDetails(self):
        ...
        self.ui.aliasLE(item.ui.labelLabel.text())
    ...
    def takeBack(self):
        ...
        reply = QWidget.QMessageBox.warning(self, i18n("Takeback operation verification"),
            ...
            i18n("If you're unsure, click Cancel and see TakeBack Plan.</center>"),
             QWidget.QMessageBox.Cancel | QWidget.QMessageBox.Ok, QWidget.QMessageBox.Ok)

        if reply == QWidget.QMessageBox.Ok:
    ...
    def takeSnapshot(self):
        reply = QWidget.QMessageBox.question(self, i18n("Start new snapshot"),
            i18n("<center>This will take a snapshot of your system.<br>Click Ok when you're ready.</center>"),
             QWidget.QMessageBox.Cancel | QWidget.QMessageBox.Ok, QWidget.QMessageBox.Ok)

        if reply == QWidget.QMessageBox.Cancel:
            return
    ...
    def saveConfig(self):
        self.settings.setValue("pos", QVariant(self.mapToGlobal(self.parent.pos())))
        self.settings.setValue("size", QVariant(self.parent.size()))
        self.settings.sync()
...
class ConfigWindow(QDialog):
    def __init__(self, parent):
        ...
        self.connect(self, pyqtSignal("accepted()"), self.saveConfig)
        self.connect(self, pyqtSignal("rejected()"), self.resetConfig)

    def saveConfig(self):
        self.settings.setValue("maxhistory", QVariant(self.ui.maxHistorySB.value()))
